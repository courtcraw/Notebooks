...
class StretchedEchellePlotter:
    def __init__(self,     
        freq, power,
        Dnu_min, Dnu_max, fmin=0, fmax=None, step=None,
                 plot_period=False, DP_min=None, DP_max=None, pstep=None,
                 d01_min=-0.5, d01_max=0.5, d01step=0.01,
                 q_min=0.001, q_max=1, q_step=0.01,
                 eps_p = 0.9,
    ...
    #==================================================================
        ...
        if self.plot_period:
            ...
            self.d01 = (d01_min + d01_max) / 2.0
            self.q = (q_min + q_max) / 2.0
    ...
    #==================================================================
        if smooth:
            self.power = StretchedEchellePlotter.smooth_power(self.power, smooth_filter_width)

        self.update_echelle()
        if self.plot_period:
            # Minimum frequency is maximum period
            self.pmin = self.f2tau(self.fmax)
            self.pmax = self.f2tau(self.fmin)
            self.period = self.f2tau(np.array(self.freq))
    ...
    #==================================================================
        ...
        if self.plot_period:
            ...
            self.create_q_slider(q_min, q_max, qstep)
            self.create_d01_slider(d01_min, d01_max, d01step)
...
#======================================================================
    ...
    def update_echelle(self):
        ...
        self.x, self.y, self.z = StretchedEchellePlotter.echelle(self.freq, self.power, self.Dnu, 
    ...
    def update_period_echelle(self):
        ...
        self.px, self.py, self.pz = StretchedEchellePlotter.echelle(self.period[::-1], self.power[::-1], 
...
#======================================================================
    ...
    def remove_point(self, f, period=False):
        ...
        min_diff = 1000
        if period:
            x0, y0 = self.p2coord(self.f2tau(f), self.DP)
        else:
        ...
        for l, ls in enumerate(self.f_labels):
            for i, freq in enumerate(ls):
                ...
                if period:
                    x, y = self.p2coord(self.f2tau(freq), self.DP)
                else:
...
#======================================================================
    ...
    def f2tau(self, freq):
        ...
        theta_p = np.pi * (freq/self.Dnu - 1/2 - self.eps_p-self.d01)
        tau = 1e6/(freq) + self.DP/np.pi * np.arctan(self.q/np.tan(theta_p))
        return tau

    def tau2f(self, period):
        """From stretched period tau (s) to frequency (muHz)"""
        # come back later
        return 1e6/period
    ...
    def save_button_clicked(self, events):
        ...
        tkinter.Tk().withdraw()
        tkinter.messagebox.showinfo("StretchedEchellePlotter", "Points saved!")
